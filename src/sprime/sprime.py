...
# Human-edited CSVs may use Unicode column/unit spellings; match via escapes (keep .py source ASCII).
_RES_COL_R2 = "R\u00b2"
_UNIT_UM_ALIAS = "\u03bc" + "m"

#: How raw responses are scaled after test/control ratio (when DMSO path applies) before Hill fit.
#: Must be set explicitly at import (:meth:`RawDataset.load_from_file`, :meth:`SPrime.load`, etc.).
ResponseNormalizationMethod = Literal["asymptote_normalized", "response_scale"]
...
def _validate_required_columns(
    ...
    has_raw_list = False
    if values_as == "list":
        has_raw_list = ("Responses" in columns_set or "responses" in columns_lower) and (
        ...
        has_raw_conc = any(
            (
                col.startswith("Conc")
                or col.startswith("CONC")
                or col.startswith("conc")
                or col.startswith("Conc")
            )
            and "Units" not in col
            and "units" not in col
            for col in columns
        )
    ...
    has_inf_asymptote = (
        "Inf_asymptote" in columns_set
        or "inf_asymptote" in columns_lower
        or "Upper" in columns_set
        or "upper" in columns_lower
        or "Infinity" in columns_set
        or "infinity" in columns_lower
    )
    has_zero_asymptote = (
        "Zero_asymptote" in columns_set
        or "zero_asymptote" in columns_lower
        or "Lower" in columns_set
        or "lower" in columns_lower
        or "Zero" in columns_set
        or "zero" in columns_lower
    )
    has_precalc_params = has_ac50 and has_inf_asymptote and has_zero_asymptote
    ...
    if not (has_raw_data_pattern or has_precalc_params):
        ...
        raise ValueError(
            ...
            f"(2) pre-calculated parameters (AC50, Zero_asymptote, Inf_asymptote or legacy Upper/Lower/Zero/Infinity). "
            f"Available columns: {available}"
        )

    # Concentration_Units required when raw data present (Path A)
    has_concentration_units = (
        "Concentration_Units" in columns_set or "concentration_units" in columns_lower
    )
    ...
    if has_raw_data_pattern and not skip_control_response_normalization:
        has_cr = any(c == "Control_Response" or c.lower() == "control_response" for c in columns)
        if not has_cr:
            available = ", ".join(sorted(columns)) if columns else "(none)"
            raise ValueError(
                f"Required column 'Control_Response' not found in {source_name}. "
                f"When raw dose-response data are present and skip_control_response_normalization=False, "
                f"each row must be able to supply a vehicle control readout via 'Control_Response'. "
                f"Available columns: {available}"
            )
...
def _convert_dataframe_to_dict_list(df) -> List[Dict]:
    ...
    try:
        ...
        raise ImportError("pandas is required to load DataFrames. Install with: pip install pandas")
...
def _validate_control_response_for_raw_row(
    row: dict,
    row_label: str,
    has_raw_curve: bool,
    *,
    ...
    cell_line_name: str,
    compound_id: str,
    skip_control_response_normalization: bool,
) -> None:
    """
    When skip_control_response_normalization is False, each raw curve row must carry a
    non-empty, non-zero numeric Control_Response (vehicle / DMSO readout).
    """
    if skip_control_response_normalization or not has_raw_curve:
        return
    cr_raw = None
    for k in row:
        if k == "Control_Response" or str(k).lower() == "control_response":
            cr_raw = row.get(k, "")
            break
    if cr_raw is None:
        cr_raw = row.get("Control_Response", "")
    s = (cr_raw if isinstance(cr_raw, str) else str(cr_raw)).strip()
    if not s:
        raise ValueError(
            f"{row_label}: Raw dose-response rows require a non-empty 'Control_Response' "
            f"(vehicle readout) when skip_control_response_normalization=False. "
            f"Compound: {compound_name}, Cell_Line: {cell_line_name}, Compound_ID: {compound_id}. "
            f"Use skip_control_response_normalization=True if responses are already control-normalized."
        )
    try:
        v = float(s)
    except (TypeError, ValueError) as e:
        raise ValueError(
            f"{row_label}: 'Control_Response' must be numeric. Got {s!r}. "
            f"Compound: {compound_name}, Cell_Line: {cell_line_name}."
        ) from e
    if v == 0.0:
        raise ValueError(
            f"{row_label}: 'Control_Response' must be non-zero. "
            f"Compound: {compound_name}, Cell_Line: {cell_line_name}."
        )


def _control_response_numeric_for_raw_row(
    row: dict,
    has_raw_curve: bool,
    skip_control_response_normalization: bool,
) -> Optional[float]:
    """
    Parsed vehicle (DMSO) control readout for automatic preprocessing before fitting.

    When ``skip_control_response_normalization`` is False and the row has raw curve data,
    returns the numeric ``Control_Response`` (validation must have run first).
    Otherwise returns None so :meth:`RawDataset.to_screening_dataset` fits raw values as-is.
    """
    if skip_control_response_normalization or not has_raw_curve:
        return None
    cr_raw = None
    ...
    if cr_raw is None:
        cr_raw = row.get("Control_Response", "")
    if cr_raw is None or cr_raw == "":
        return None
    if isinstance(cr_raw, bool):
        pass  # fall through to str()
    else:
        try:
            import numbers

            if isinstance(cr_raw, numbers.Real):
                return float(cr_raw)
