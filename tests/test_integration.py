...
class TestCompleteWorkflow:
    ...
    def test_complete_workflow_single_compound_multiple_cell_lines(self):
        ...
        try:
            ...
            delta_results = screening_data.calculate_delta_s_prime(
                reference_cell_lines="Reference Cell Line", test_cell_lines="Test Cell Line"
            )
    ...
    def test_delta_s_prime_export(self):
        ...
        try:
            ...
            delta_results = screening_data.calculate_delta_s_prime(
                reference_cell_lines="Reference", test_cell_lines="Test"
            )
    ...
    def test_assay_timescale_propagates_raw(self):
        ...
        try:
            ...
            assert (
                ref_prof.metadata is not None and ref_prof.metadata.get("assay_timescale") == "24hr"
            )
            assert (
                test_prof.metadata is not None
                and test_prof.metadata.get("assay_timescale") == "48hr"
            )
...
class TestAllowOverwritePrecalcParams:
    """Test allow_overwrite_precalc_params: raise when disallowed, succeed + warn when allowed."""
    ...
    def test_allow_overwrite_precalc_params_raises(self):
        ...
        try:
            ...
            assert len(raw_data) == 1
            with pytest.raises(ValueError, match="allow_overwrite_precalc_params"):
                SPrime.process(raw_data, allow_overwrite_precalc_params=False)
        finally:
            csv_file.unlink()

    def test_allow_overwrite_precalc_params_succeeds(self):
        """With raw + pre-calc, process(allow_overwrite=True) succeeds and logs OVERWRITE_PRECALC_PARAMS."""
        rows = [
            {
                "Compound Name": "Drug A",
                "Compound_ID": "DRUG001",
        ...
        try:
            ...
            screening_data, process_report = SPrime.process(
                raw_data, allow_overwrite_precalc_params=True
            )
            assert len(screening_data) == 1
            profile = list(screening_data.profiles)[0]
            assert profile.hill_params is not None
            assert profile.s_prime is not None
            overwrite_warnings = [
                w for w in process_report.warnings if w.category == "OVERWRITE_PRECALC_PARAMS"
            ]
...
class TestErrorHandling:
    ...
    def test_workflow_with_missing_cell_line(self, tmp_path):
        ...
        results = screening_data.calculate_delta_s_prime(
            reference_cell_lines="Cell1", test_cell_lines="NonExistent"
        )
    ...
    def test_values_as_list_load_process(self):
        """Load demo_precontrol_normalized_raw_list with values_as='list', process, check S'."""
        from pathlib import Path
    ...
    def test_demo_raw_vehicle_control_load_strict(self):
        """Raw demo with Control_Response=35.3 loads under default strict import."""
        from pathlib import Path
    ...
    def test_precontrol_normalized_raises_when_control_required(self):
        """Empty Control_Response fails on raw rows under default strict import."""
        from pathlib import Path
        ...
        with pytest.raises(ValueError, match="Control_Response"):
            SPrime.load(
                csv_path,
                values_as="columns",
                response_normalization="asymptote_normalized",
...
class TestCalculateDeltaSPrimeUtility:
    ...
    def test_calculate_delta_s_prime_with_list_dict(self):
        ...
        results = calculate_delta_s_prime(
            list_of_dicts, reference_cell_line_names="Reference", test_cell_line_names="Test"
        )
